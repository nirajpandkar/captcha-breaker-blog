import flask
from predict import predict_captcha_patna, predict_captcha_allahabad, predict_captcha_lucknow, predict_captcha_sc, predict_captcha_gujrat, predict_captcha_telangana
import numpy as np
import io
    ...
    try:
        if flask.request.method == "POST":
            if flask.request.files.get("image"):
                ...
                predicted_captcha = predict_captcha_telangana(open_cv_image)
                logging.info("Telangana Captcha: {}".format(predicted_captcha))
                if predicted_captcha:
                    data["predicted_captcha"] = predicted_captcha
                    data["success"] = True
        return flask.jsonify(data)
    ...
    try:
        if flask.request.method == "POST":
            if flask.request.files.get("image"):
                ...
                predicted_captcha = predict_captcha_gujrat(open_cv_image)
                logging.info("Gujrats Captcha: {}".format(predicted_captcha))
                if predicted_captcha:
                    data["predicted_captcha"] = predicted_captcha
                    data["success"] = True
        return flask.jsonify(data)
